...
.then(data=>{chartForObjects(data,titleText,yAxisText,seriesName);
})
}
...
function displayChartForNestedObject(fileName,titleText,yAxisText,seriesName){
    fetch('../output/'+fileName+'.json')
    .then(response=>response.json())
    .then(data=>{chartForNestedObjects(data,titleText,yAxisText,seriesName);
    })
    }
